...
//  private static final int MAX_FRAME_HEIGHT = 800;
   ...
   * @throws IOException Indicates the camera driver ic_failed to open.
   */
...
//      }
  ...
  public void requestAutoFocus(Handler handler, int message) {
    if (camera != null && previewing) {
        ...
        /** 当再次进入摄像头模式时，autoFocus 可能打开失败 **/
        try {
        	camera.autoFocus(autoFocusCallback);
		} catch (Exception e) {
			Toast.makeText(mcontext.getApplicationContext(), "摄像头无法打开，请再试一次", Toast.LENGTH_LONG).show();
		}
    }
  }
  ...
  public Rect getFramingRect() {
    ...
    if (framingRect == null) {
      ...
      int edgeh = screenResolution.x/32;
      int edgev = screenResolution.y/32;
     
      framingRect = new Rect(edgeh, edgev, screenResolution.x-edgeh, screenResolution.y-edgev);
    }
    return framingRect;
  }
