...
public class WheelView extends View {
    ...
    /** Cutting Line color */
    private static final int CUTTING_LINE_COLOR = 0xFFaaaaaa;
...
//    private static final int[] SHADOWS_COLORS = new int[] { 0xFF111111, 0x00AAAAAA, 0x00AAAAAA };
    ...
    private  int ADDITIONAL_ITEM_HEIGHT = (int) (textSize*1.5);
    ...
    // Wheel Values
    protected WheelAdapter adapter = null;
    ...
    private Paint cuttingLinePaint;
...
//    private GradientDrawable bottomShadow;
    ...
    // Listeners
    protected List<OnWheelChangedListener> changingListeners = new LinkedList<OnWheelChangedListener>();
    ...
    private void initResourcesIfNecessary() {
        ...
        if (cuttingLinePaint == null) {
        	//设置
        	cuttingLinePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
            // valuePaint.density = getResources().getDisplayMetrics().density;
            cuttingLinePaint.setColor(CUTTING_LINE_COLOR);
        }
...
//        setBackgroundResource(R.drawable.bg_wheel);
    ...
    private int calculateLayoutWidth(int widthSize, int mode) {
        ...
        if (maxLength > 0) {
        	//计算字符宽度
            float textWidth = (float) Math.ceil(Layout.getDesiredWidth("宽", itemsPaint));
...
//    }
    ...
    private void drawCuttingLine(Canvas canvas){
    	cuttingLineWidth = (int) (getWidth()*(5/12.0f)); 
    	int centerY = getHeight()/2;
    	int offsetY = getItemHeight()/2;
    	int centerX = getWidth()/2;
    	int offsetX = cuttingLineWidth/2;
    	canvas.drawLine(centerX - offsetX, centerY - offsetY, centerX + offsetX,centerY - offsetY, cuttingLinePaint);
    	canvas.drawLine(centerX - offsetX, centerY + offsetY, centerX + offsetX,centerY + offsetY, cuttingLinePaint);
